...
export function registerCourseHandlers(): void {
  ipcMain.handle('courses:list', (_e, semesterId: number) => {
    const db = getDb()
  ...
  ipcMain.handle('courses:update', (_e, payload) => {
    const db = getDb()
    const { id, schedule, ...upd } = payload as { id: number; schedule?: unknown[] } & Record<string, unknown>
    ...
    if (schedule !== undefined) {
      ...
      const ins = db.prepare('INSERT INTO weekly_schedule (course_id, day_of_week, start_time, end_time) VALUES (?, ?, ?, ?)')
      for (const slot of schedule as Array<{ dayOfWeek: number; startTime: string; endTime: string }>) ins.run(id, slot.dayOfWeek, slot.startTime, slot.endTime)
    }
    return getCWS(id)
  })
